    ...
    <div id="sidebar-wrapper">
        <div class="well sidebar-nav">
		 ...
		 <footer class="footFix footRight">
			<span style="display:block;width:50px;height:50px;background:url(${APP_PATH}/static/home/images/cart.png)">
				&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<sup id="total-sum" display="font-color:red;">0</sup>
...
<div id="wrapper2" class="viewer wrapper countpage clearfix" style="display:none">
    ...
    <section class="oder_submit">
        <div class="container">
            <a  href="${APP_PATH}/account/index" class="button button-rounded button-flat-action fr mgtbr8">
                                  订单中心
            </a>
                    ...
                    <form role="form">
                        ...
                        <div class="form-group" id="form_info">
                            ...
                            <span class="twitter-typeahead">
                                <input type="text" name="phone" id="add-phone" maxlength="11" class="form-control tt-query" autocomplete="off"
                            ...
                            <span class="twitter-typeahead">
                                <input type="text" name="receiveName" id="add-receiveName" class="form-control tt-query" autocomplete="off"
                                spellcheck="false" dir="auto">
                            </span>
                            ...
                            <span class="twitter-typeahead">
                                <input type="text" name="adress" id="add-address" class="form-control tt-query" autocomplete="off"
                                spellcheck="false" dir="auto">
                            </span>
                        </div>
                        <div class="form-group">
                            <div class="btn_control fr">
                                ...
                                <button type="button" class="btn btn-info bottommargin" id="submit-order-action">
                                                                            确认
                                </button>
                            </div>
                        </div>
...
<div id="loadingView" class="viewer">
  ...
  <script src="${APP_PATH}/static/home/js/jquery.fly.min.js"></script> 
  ...
  <script>
		...
		$(function(){
			...
			$.ajax({
						url:"${APP_PATH}/index/getFoodList",
						type:"post",
						dataType:"json",					
		                success:function(result){
		                	if(result.success){
								iG.items = result.data;//获取后端传过来的数据
								init();//初始化
								showTotal();
								showEveryFoodTotal();
		                	}else{
			...
			$("#submitOrder").click(function(){
				//向后台发送请求获取客户数据
				$.ajax({
					url:"${APP_PATH}/account/getUserAddress",
					type:"POST",
					success:function(result){
						if(result.success){
							//进行数据的回显
							$('#add-phone').val(result.data.phone);
							$('#add-receiveName').val(result.data.realName);
							$('#add-address').val(result.data.address);
			...
			$('#submit-order-action').click(function(){
				...
				var receiveName = $('#add-receiveName').val();
				...
				//获取下单的菜品
				var foodList = iG.order;
				var ids = "";
				for(var i in foodList){
					if(foodList[i].counter===0)continue;
					ids+=foodList[i].id+","+foodList[i].counter+";";
				}
				if(ids==""){
					alert("请选择菜品下单！");
					return;
				...
				$.ajax({
					url:"${APP_PATH}/order/submitOrder",
					type:"POST",
					data:{
						"phone":phone,
					...
					success:function(result){
						if(result.success){
							...
							$('#clearOrder').click();
							//定位到用户订单中心
							window.location.href="${APP_PATH}/account/index";
						}else{
						  alert(result.data);	
						}
					}
